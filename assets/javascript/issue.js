$(document).ready(function () {
    ...
    $.getJSON("/getemployeejson", { ajax: true }, function (data) {
        ...
        $.each(data, function (index, item) {
            $('#demand_employeeid').append($('<option>').text(item[1]).val(item[0]))
        })
    })
    ...
    $('#btnsubmit').click(function () {
        $.getJSON("/displayissuealljson", { fromdate: $('#fromdate').val(),todate: $('#todate').val() }, function (data) {
            ...
            $.each(data, function (index, item) {
              htm += "<tr><th scope='row'>"+item.issueid+"</th><td>"+item.categoryname+"</td><td>"+item.subcategoryname+"</td><td>"+item.productname+"</td><td>"+item.finalproductname+"</td><td>"+item.dfname+" "+item.flname+"</td><td>"+item.dateissue+"</td><td>"+item.qtyissue+"</td><td>"+item.remark+"</td></tr>"
            })
            htm+= "</table>"
            $('#result').html(htm)
        })
